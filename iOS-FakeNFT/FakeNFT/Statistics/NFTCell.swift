...
extension NFTCell {
    func configure(with nft: ItemNFT?) {
        ...
        ratingView.setupRating(rating: nft?.rating ?? .zero)
        
        priceLabel.text = if let price = nft?.price {
            "\(price) ETH"
        } else {
            "?"
        }
        
        guard let url = nft?.images.first, let validUrl = URL(string: url) else { return }
